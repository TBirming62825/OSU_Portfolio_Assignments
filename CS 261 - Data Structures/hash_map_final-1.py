...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        for linked_list in range(self.capacity):
            if self._buckets[linked_list].size >= 1:
                curr_node = self._buckets[linked_list].head
                while curr_node is not None:
                    hash_index = self._hash_function(curr_node.key)
                    temp._buckets[hash_index].add_front(curr_node.key, curr_node.value)
